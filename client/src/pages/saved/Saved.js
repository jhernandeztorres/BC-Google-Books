...
class Saved extends Component {
    ...
    loadBooks = () => {
        API.getBooks().then(res => this.setState({ books: res.data, title: "", author: "", description: ""})).catch((error) => {console.log(error)})
    };

    deleteBook = id => {
        API.deleteBook(id).then(res => this.loadBooks()).catch(error => console.log(error));
    }
    ...
    handleFormSubmit = event => {
        ...
        if (this.state.title && this.state.author) {
            ...
            }).then(res => this.loadBooks()).catch(error => console.log(error))
        }
    };
        ...
        <Row>
          <Col size="md-6">
            ...
            <form>
              ...
              <FormBtn disabled={!(this.state.author && this.state.title)} onClick={this.handleFormSubmit}>Submit</FormBtn>
            </form>
          </Col>
          <Col size="md-6 sm-12">
            <Jumbotron>
